...
import ru.imaginaerum.wd.common.init.items.entity.ModChestBoatEntity;
...
public class ModBoatRenderer extends BoatRenderer {
    ...
    public ModBoatRenderer(EntityRendererProvider.Context pContext, boolean pChestBoat) {
        ...
        this.boatResources = Stream.of(ModBoatEntity.Type.values()).collect(
                ImmutableMap.toImmutableMap(
                        type -> type,
                        type -> Pair.of(
                                ResourceLocation.fromNamespaceAndPath(WD.MOD_ID, getTextureLocation(type, pChestBoat)),
    ...
    private ListModel<Boat> createBoatModel(EntityRendererProvider.Context pContext, ModBoatEntity.Type pType, boolean pChestBoat) {
        ModelLayerLocation layerLocation = pChestBoat
                ? createChestBoatModelName(pType)
                : createBoatModelName(pType);
        ModelPart modelPart = pContext.bakeLayer(layerLocation);
    ...
    private static ModelLayerLocation createLocation(String pPath, String pModel) {
        return new ModelLayerLocation(ResourceLocation.fromNamespaceAndPath(WD.MOD_ID, pPath), pModel);
    }
